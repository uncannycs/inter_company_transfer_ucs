...
class PurchaseOrder(models.Model):
    ...
    def _compute_intercompany_transfer_count(self):
        for order in self:
            if order.intercompany_transfer_id:
                ...
                order.intercompany_transfer_count = self.env['inter.company.transfer'].search_count([('purchase_order_ids', 'in', order.id)])
    ...
    def button_confirm(self):
        ...
        for order in self:
            ...
            if company.intercompany_apply_type in ('purchase', 'both'):
                ...
                if target_company and target_company != company and not order.intercompany_transfer_id:
                    # Target company is Selling to this Company
                    from_wh = target_company.intercompany_warehouse_id or self.env['stock.warehouse'].search([('company_id', '=', target_company.id)], limit=1)
                    to_wh = order.picking_type_id.warehouse_id or company.intercompany_warehouse_id or self.env['stock.warehouse'].search([('company_id', '=', company.id)], limit=1)

                    ict_vals = {
                        'from_company_id': target_company.id,
                        'from_warehouse_id': from_wh.id if from_wh else False,
                        'to_company_id': company.id,
                        'to_warehouse_id': to_wh.id if to_wh else False,
                        'purchase_order_ids': [(4, order.id)],
                        'line_ids': [(0, 0, {
                    ...
                    # Create SO in target_company
                    so_vals = {
                        'partner_id': company.partner_id.id,
                        'company_id': target_company.id,
                        'warehouse_id': from_wh.id if from_wh else False,
                    ...
                    so = self.env['sale.order'].with_company(target_company).with_context(skip_intercompany_auto=True).create(so_vals)
                    so.with_company(target_company).action_confirm()
                    ict.sale_order_ids = [(4, so.id)]
                    ...
                    if target_company.create_invoice_bill:
                        so_invs = so.with_company(target_company)._create_invoices()
                        for inv in so_invs:
                            ...
                            if target_company.validate_invoice_bill and inv.state == 'draft':
                                inv.with_company(target_company).action_post()

                    if company.create_invoice_bill:
                        bill_vals = order.with_company(company)._prepare_invoice()
                        bill_vals['invoice_date'] = fields.Date.context_today(self)
                        bill_vals['intercompany_transfer_id'] = ict.id if company.link_intercompany_document else False
                        bill_vals['invoice_line_ids'] = [(0, 0, po_line._prepare_account_move_line()) for po_line in order.order_line if not po_line.display_type]
                        bill = self.env['account.move'].with_company(company).create(bill_vals)
                        ict.invoice_ids = [(4, bill.id)]
                        if company.validate_invoice_bill and bill.state == 'draft':
                            bill.with_company(company).action_post()
    ...
    def action_view_intercompany_transfer(self):
        self.ensure_one()
        transfers = self.env['inter.company.transfer'].search(['|', ('id', '=', self.intercompany_transfer_id.id), ('purchase_order_ids', 'in', self.id)])
        return {
